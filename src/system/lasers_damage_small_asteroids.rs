...
use crate::component::{Laser, SmallAsteroid, Collidable};
...
impl<'a> System<'a> for LasersDamageSmallAsteroids {
    ...
    fn run(&mut self, (entities, small_asteroids, lasers, transforms, collidables): Self::SystemData) {
        for (small_asteroid_entity, _small_asteroid, small_asteroid_local, small_asteroid_collidable) in
            (&entities, &small_asteroids, &transforms, &collidables).join()
        {
            for (_laser, laser_entity, laser_local, laser_collidable) in (&lasers, &entities, &transforms, &collidables).join() {
