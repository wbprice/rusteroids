...
use amethyst::{
    ...
    ecs::{Entities, Entity, Join, System, ReadStorage, WriteStorage},
    renderer::{
...
impl<'a> System<'a> for DebugBoxes {
    ...
    fn run(&mut self, (entities, players, mut debug_lines, transforms): Self::SystemData) {

        let mut debug_lines_to_update : Vec<(Entity, DebugLinesComponent)> = vec![];
        
        // Update debug components for the Player
        for (entity, _player, _debug_line, local) in (&entities, &players, &debug_lines, &transforms).join() {
        ...
        for (entity, _asteroid, _debug_line, local) in (&entities, &players, &debug_lines, &transforms).join() {
